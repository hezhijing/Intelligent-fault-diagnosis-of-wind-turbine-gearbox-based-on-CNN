# -*- coding: utf-8 -*-
"""
Created on Wed Jun 19 09:46:05 2019
...
def read_data(path):
    ...
    for idx, folder in enumerate(cate):
        for t in glob.glob(folder + '\*.mat'):
            ...
            mat = loadmat(t)
            mat_data = np.append(mat['Var_1'], mat['Var_2'], axis = 0)
            mat_data = np.append(mat_data, mat['Var_3'], axis = 0)
...
def data_preprocessing(data, label, test_size):
...
if __name__ == '__main__':
    ...
    x_train, x_test, y_train, y_test = data_preprocessing(data, label, test_size)
    x_train = x_train[:, :, np.newaxis]
    x_test = x_test[:, :, np.newaxis]
    
    #-----------------构建网络----------------------
    x = tf.placeholder(tf.float32, shape=[None, 10, 1], name='x')
